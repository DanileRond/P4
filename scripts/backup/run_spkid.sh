...
for filename in $(cat $lists/class/all.train $lists/class/all.test); do
         mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2lpcc 12 10 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
   done
}
compute_mfcc() {
for filename in $(cat $lists/class/all.train $lists/class/all.test); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2mfcc 13 10 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
   done
}
   ...
   if [[ $cmd == train ]]; then
	...
	# \DONE
       ...
       for dir in $db/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           gmm_train  -v 1 -T 0.001 -N5 -m 5 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
	...
	#gmm_train  -v 1 -t 0.01 -n5 -m 5 -d $w/$FEAT -e $FEAT -g $w/gmm/world/world.gmm $lists/verif/users_and_others.train || exit 1
	#gmm_train  -v 1 -T 0.001 -N5 -m 5 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
	gmm_train -v 1 -t 0.01 -n15 -m32 -d $w/lists_verif/users_and_others.train || exit 1
# \DONE
	...
	# \DONE
	gmm_verify -d $w/mcp -e mcp -D $w/gmm/mcp -E gmm -w world $w/lists/gmm.list $w/lists_verif/all.test  $w/lists_verif/all.test.candidates > $w/spk_verify.log || exit 1
